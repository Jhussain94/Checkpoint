...
function ccBuildGrid() {
  ...
  const uploads = typeof getCampusCameraUploads === 'function'
    ? getCampusCameraUploads()
    : [];
  ...
  grid.innerHTML = Array.from({ length: 6 }, (_, i) => {
    ...
    const alt = entry.caption ? entry.caption : 'Campus photo';
    return `<div class="${slotClass}">
              <img src="${entry.imageUrl}" alt="${alt.replace(/"/g, '&quot;')}" loading="lazy">
            </div>`;
  }).join('');
}
