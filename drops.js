// Mock drop data — replace getDrops() with a Supabase query when ready
const DROPS = [
  // ── MEDIA MEADOW ──────────────────────────────────────────────────────────
  {
    id: 'm1',
  ...
  },
  {
    ...
    mood: 'peaceful',
    area: 'meadow',
    timestamp: '2024-08-15T03:12:00Z',
    username: 'anonymous',
    position: { x: 75, y: 38 }
  },
  ...
  },
  {
    ...
    mood: 'nostalgic',
    area: 'garden',
    timestamp: '2024-06-14T19:45:00Z',
    username: 'illustration yr3',
    position: { x: 28, y: 14 }
  },
  ...
  },
  {
    ...
    mood: 'chaotic',
    area: 'workshop',
    timestamp: '2024-04-02T22:15:00Z',
    username: 'anonymous',
    position: { x: 55, y: 62 }
  },
  {
  ...
  // ── LOST & FOUND FOREST ───────────────────────────────────────────────────
  {
    ...
    mood: 'lonely',
    area: 'forest',
    timestamp: '2024-06-01T03:20:00Z',
    username: 'anonymous',
    position: { x: 18, y: 62 }
  },
  {
  ...
  },
  {
    ...
    mood: 'hopeful',
    area: 'forest',
    timestamp: '2025-01-19T00:15:00Z',
    username: 'anonymous',
    position: { x: 8, y: 80 }
  },
  ...
  },
  {
    ...
    mood: 'hopeful',
    area: 'corner',
    timestamp: '2024-12-09T23:59:00Z',
    username: 'anonymous',
    position: { x: 84, y: 72 }
  },
  {
...
// Supabase hook: replace this function body with:
//   const { data } = await supabase.from('drops').select('*');
//   return data;
function getDrops() {
  const userDrops = JSON.parse(sessionStorage.getItem('userDrops') || '[]');
  return [...DROPS, ...userDrops];
}

function saveUserDrop(drop) {
  const existing = JSON.parse(sessionStorage.getItem('userDrops') || '[]');
  existing.push(drop);
  sessionStorage.setItem('userDrops', JSON.stringify(existing));
}
